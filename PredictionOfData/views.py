...
def Dashboard(request):
    if request.method == 'POST':
        try:
            admin_user = userMaster.objects.get(
                email=request.POST['admin_email'], password=request.POST['admin_password'])
...
def FileUploded(request):
    if request.method == "POST":
        ...
        newSplitedFileName = str(file).split(".")
        global newFileName
        newFileName = newSplitedFileName[0] + "-UpdatedFile.xlsx"
